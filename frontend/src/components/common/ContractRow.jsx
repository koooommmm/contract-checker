...
const ContractRow = (props) => {
    ...
    return <tr key={props.index}>
        <td className='p-2 w-2/3'>
            ...
            { isEditing == false ? 
                /* 詳細へのリンク */
                <button onClick={() => {navigate('/contract/' + props.id)}} className='float-left'>
                    {title}
                </button> :
                /* 編集中 */
            ...
            { isEditing == false ?
                /* 編集ボタン */
                <button onClick={() => setEditingIndex(props.index)} className='float-right'>
                    <HiPencil size={20} color={'#888'}/>
                </button> :
                /* 決定ボタン・取り消しボタン */
                <>
                <button onClick={() => {setEditingIndex(-1); setTitle(props.title)}} className='float-right'>
                    <FaTimes size={20} color={'#888'}/>
                </button>
                <button onClick={async () => {await editTitle(props.id, title); setEditingIndex(-1)}} className='float-right'>
